...
3. **§二/§三/§四/§五/§六 术语本地化**：Electron 三层进程模型、[docs/](docs/文档索引.md) 设计文档体系、M1-M23 模块编号、zod 契约边界。
4. **§十一 Graph Engineering**：按项目规模裁剪（当前单仓库、规划模块 23 个 <50；重启评估触发条件见 §十一 开头）。
...
**项目落地**：决策留痕体系已建于 [docs/](docs/文档索引.md)——核心决策 36 项见 [docs/decisions/决策索引](docs/decisions/决策索引.md)，契约内决策 68 项见各契约文档末尾决策表；任务与 wave 跟踪见 [docs/04-模块全景 §4](docs/04-模块全景.md#4-v01-分波次交付计划6-8-周) 与 [docs/07-立项准备](docs/07-立项准备.md)。新增决策一律附带「否决方案理由」并分配系列 ID（D/T/IP/CP/TP/SP/PC/DT/ST/OM/TH/OR/SC）。
...
> **适配留痕（§〇.5 要求）**：本节数值的**单一真源**为 [docs/03-技术栈 §4](docs/03-技术栈.md#4-测试覆盖率适配门槛)，此处为同步摘录；若两处漂移，以该文档为准。
...
| E2E 关键路径                                         | ≥100%（每路径至少一条） | 启动 / 导航 / 开关标签 / AI 摘要 各至少 1 条正路 | 全覆盖    | v0.2 起增加隐私拦截 / 扩展加载路径                                      |
...
| 文件命名  | 单元 `*.test.ts` / 集成 `*.integration.test.ts` / E2E `*.e2e.ts`                                                                        |
...
| 提交时           | pre-commit（Husky + lint-staged） | ESLint → `tsc --noEmit` → Prettier → 相关单测，任一失败阻止提交                                          |
...
> ✂ **本项目裁剪决定（2026-07-29）**：Urchin Browser 当前为 pre-code 单仓库，规划模块 23 个（M1-M23）<50，按上方条款**裁剪本节**，当前项目规模不适用。**触发重启评估的条件**：① 代码模块数 >50；② 拆分为多仓库；③ v0.5+ 跨包依赖治理出现实际痛点。任一条件触发时须重新评估引入图谱机制，不可跳过决策直接忽略。
...
| 文档条款          | 通用模板，放入项目后须适配但不得违背架构 | 适配义务 / 修改红线 / 权限流程 / 产出留痕                                       | — 未修改                                  |
| 总纲              | AI 全流程驱动，人类设目标验结果          | 目标前置/小步快跑/时时记录/独立验证/不确定性上报/上下文边界/不可逆操作审批/自审 | — 未修改                                  |
...
| Code Review       | 三步自审法（可按变更类型匹配轮次）       | 性能 → 逻辑架构 → 健壮性                                                        | ✓ 已适配（项目特化审查点）                |
| 代码注释          | 三级注释记录设计理由                     | 必须级4条 + 推荐级4条，记 why 不记 what                                         | — 未修改                                  |
| 测试覆盖          | 分层测试 + 动态覆盖率门槛                | 默认 ≥80%，验收即测试                                                           | ✓ 已适配（门槛阶梯化，留痕见 §八.2）      |
| 质量护栏          | 自动化检查关卡拦坏代码                   | 写时/提交时/PR/不可逆操作门禁/发布前                                            | ✓ 已适配（工具链项目化）                  |
